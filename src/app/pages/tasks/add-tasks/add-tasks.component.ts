...
export class AddTasksComponent implements OnInit, OnDestroy {
  ...
  assignedToList: UserDetailDTO[] = [];
  assignedByList: UserDetailDTO[] = [];
  ...
  ngOnInit(): void {
    ...
    this.contactSubject = this._empService.customerObserver$.subscribe(res => {
      if (res) {
    ...
    this.prioritySubject = this._empService.getPriorities().subscribe(res => {
      if (res)
        this.priorities = res.dynamicResult as {id:number,name:string}[];
    });
